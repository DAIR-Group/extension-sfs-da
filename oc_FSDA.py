...
def run(k):
    try:
        ...
        Xs, ys, mu_s, Sigma_s = ElasticNet.gen_data(ns, p, true_beta)
        Xt, yt, mu_t, Sigma_t = ElasticNet.gen_data(nt, p, true_beta)
        ...
        fs_model = ElasticNet(X_tilde, y_tilde, Lambda, Gamma)
        M = fs_model.fit()
        fs_model.check_KKT()
